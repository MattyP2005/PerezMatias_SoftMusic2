...
namespace HerramientasdeProgramacion.API.Controllers
{
    ...
    public class CancionesController : ControllerBase
    {
        ...
        public IActionResult DescargarCancion(int id)
        {
            ...
            if (!PlanServices.PermiteDescargas(usuario.Plan) || usuario.Plan == "Free")
            ...
            // Construir ruta del archivo mp3, asumiendo que c.Url es el nombre relativo del archivo
            var filePath = Path.Combine(_env.WebRootPath, "canciones", cancion.Url);

            if (!System.IO.File.Exists(filePath))
                return NotFound("Archivo de canción no encontrado.");

            var mimeType = "audio/mpeg";
            var fileName = $"{cancion.Titulo}.mp3";
